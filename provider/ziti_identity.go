...
type IdentityEnrollmentsOtt struct {
	...
	// id
	ID string `pulumi:"zitiId,optional"`
...
func (Identity) Create(ctx context.Context, req infer.CreateRequest[IdentityArgs]) (infer.CreateResponse[IdentityState], error) {
	input := req.Inputs
	name := req.Name
	preview := req.DryRun
	...
	retErr := func(err error) (infer.CreateResponse[IdentityState], error) {
		return infer.CreateResponse[IdentityState]{Output: IdentityState{IdentityArgs: input}}, err
	}
	...
	var enrollment *rest_model.IdentityCreateEnrollment
	if input.Enrollment != nil {
	...
	if err != nil {
		...
		if errors.As(err, &badReq) {
			...
			updatedResp, err := identity.Update(ctx, infer.UpdateRequest[IdentityArgs, IdentityState]{ID: existingId, State: state, Inputs: input, DryRun: preview})
				return infer.CreateResponse[IdentityState]{ID: existingId, Output: updatedResp.Output}, err
			}
			return retErr(err2)
		}
	...
	if err != nil {
		return infer.CreateResponse[IdentityState]{ID: createdId, Output: state}, err
	}
	return infer.CreateResponse[IdentityState]{ID: createdId, Output: state}, nil
}
	...
	return IdentityState{
		...
		Ottca: iftfden(data.Enrollment.Ottca != nil, func() IdentityEnrollmentsOttca {
			return IdentityEnrollmentsOttca{
				Ca:        iftfden(data.Enrollment.Ottca.Ca != nil, func() EntityRef { return buildEntityRef(data.Enrollment.Ottca.Ca) }),
...
func (Identity) Read(ctx context.Context, req infer.ReadRequest[IdentityArgs, IdentityState]) (infer.ReadResponse[IdentityArgs, IdentityState], error) {
	ce, _, err := initClient(ctx)
	if err != nil {
		return infer.ReadResponse[IdentityArgs, IdentityState]{ID: req.ID, Inputs: req.Inputs, State: req.State}, err
	}
	readState, err := readIdentity(ce, req.ID, req.Inputs, req.State.Assimilated)
	if err != nil {
		return infer.ReadResponse[IdentityArgs, IdentityState]{ID: req.ID, Inputs: req.Inputs, State: readState}, err
	}
	return infer.ReadResponse[IdentityArgs, IdentityState]{ID: req.ID, Inputs: req.Inputs, State: readState}, nil
}
...
func (Identity) Update(ctx context.Context, req infer.UpdateRequest[IdentityArgs, IdentityState]) (infer.UpdateResponse[IdentityState], error) {
	ce, _, err := initClient(ctx)
	if err != nil {
		return infer.UpdateResponse[IdentityState]{Output: req.State}, err
	}
	updateData := &rest_model.IdentityUpdate{
		AppData:                  buildZitiTags(req.Inputs.AppData),
		AuthPolicyID:             &req.Inputs.AuthPolicyID,
		DefaultHostingCost:       &req.Inputs.DefaultHostingCost,
		DefaultHostingPrecedence: req.Inputs.DefaultHostingPrecedence,

		ExternalID:                req.Inputs.ExternalID,
		IsAdmin:                   &req.Inputs.IsAdmin,
		Name:                      &req.Inputs.Name,
		RoleAttributes:            &req.Inputs.RoleAttributes,
		ServiceHostingCosts:       req.Inputs.ServiceHostingCosts,
		ServiceHostingPrecedences: req.Inputs.ServiceHostingPrecedences,
	...
	// bail out now when we are in preview mode
	if req.DryRun {
		return infer.UpdateResponse[IdentityState]{Output: req.State}, nil
	}
	...
	if err != nil {
		...
		if errors.As(err, &badReq) {
			return infer.UpdateResponse[IdentityState]{Output: req.State}, formatApiErr(ctx, badReq, badReq.Payload)
		}
		return infer.UpdateResponse[IdentityState]{Output: req.State}, err
	}
	...
	readState, err := readIdentity(ce, req.ID, req.Inputs, req.State.Assimilated)
	if err != nil {
		return infer.UpdateResponse[IdentityState]{Output: readState}, err
	}
	return infer.UpdateResponse[IdentityState]{Output: readState}, nil
}
...
func (Identity) Delete(ctx context.Context, req infer.DeleteRequest[IdentityState]) (infer.DeleteResponse, error) {
	...
	if req.State.Assimilated && !c.deleteAssimilated {
		p.GetLogger(ctx).Infof("DELETE on %s[%s]: Keeping on OpenZiti as this object was assimilated!", "Identity", req.ID)
