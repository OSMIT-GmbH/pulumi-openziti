...
func (c *OpenZitiProviderConfig) Configure(ctx context.Context) error {
...
func dumpStruct(ctx context.Context, name string, data interface{}) {
	out, err := json.Marshal(data)
	if err != nil {
		p.GetLogger(ctx).Infof("ERROR: failed serializing data: %s => %#v", err.Error(), data)
	} else {
		p.GetLogger(ctx).Infof("OK: %s(json)=%s", name, string(out))
	}
}
...
func initClient(ctx context.Context) (*CacheEntry, OpenZitiProviderConfig, error) {
...
func formatApiErr(ctx context.Context, err error, apiError *rest_model.APIErrorEnvelope) error {
	// if errors.Is(err, config.CreateConfigBadRequest) {
	errOut, err2 := json.Marshal(apiError.Error)
	if err2 != nil {
		p.GetLogger(ctx).Errorf("ERROR: type: ErrorString: %v, MarshallingError: %s Payload.Error=%+v  PalyloadErrorCause=%#v PayloadMetadata: %+v\n", err.Error(), err2.Error(), apiError.Error, apiError.Error.Cause, apiError.Meta)
		return errors.Join(err, err2)
	}
	return fmt.Errorf("ERROR: type: ErrorString: %s, Payload=%s\n", err.Error(), string(errOut))
}
func formatApiErrDupeCheck(ctx context.Context, err error, apiError *rest_model.APIErrorEnvelope) (error, bool) {
...
func handleDeleteErr(ctx context.Context, err error, id string, typeName string) error {
	var apiError *runtime.APIError
	if errors.As(err, &apiError) {
		if apiError.Code == 404 {
			p.GetLogger(ctx).Warningf("DELETE on %s[%s] returned 404 - assuming already deleted!", typeName, id)
			return nil
		}
	}
	return err
...
func diffWalk(ctx context.Context, diff map[string]p.PropertyDiff, path string, old reflect.Value, new reflect.Value) {
	p.GetLogger(ctx).Debugf("diffWalk: visiting %s: old: %s new: %s", path, old.String(), new.String())
	...
	if new.Kind() != old.Kind() {
		p.GetLogger(ctx).Infof("diffWalk: visiting %s: Kind changed: old: %s new: %s", path, old.Kind().String(), new.Kind().String())
	...
	default:
		// handle other types
		diff[path] = p.PropertyDiff{Kind: p.Update}
		p.GetLogger(ctx).Warningf("Unhandled types comparing %s: %s<>%s, %s != %s", path, old.Kind().String(), new.Kind().String(), old.String(), new.String())
	}
}

func diffStrArrayIgnoreOrder(ctx context.Context, diff map[string]p.PropertyDiff, path string, old []string, new []string) {
	oldLen := len(old)
	newLen := len(new)
